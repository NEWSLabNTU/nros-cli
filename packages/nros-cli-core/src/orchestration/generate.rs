...
fn render_rust_toolchain(plan: &NrosPlan) -> Option<String> {
    // Phase 126.M5.esp32 — ESP32-C3 (riscv32imc) needs nightly +
    // rust-src for `-Z build-std`. Pin matches `tools/rust-toolchain.toml`
    // (the channel `just esp32 build-examples` passes as `+<nightly>`).
    // NOTE: ESP32-S3 (Xtensa) needs the `+esp` espup toolchain instead;
    // not emitted here — board="esp32s3" support is a follow-up.
    if let Some(chip) = esp32_chip(&plan.build.board) {
        if chip == "esp32c3" {
            return Some(
...
"#
        ...
        // esp32s3 → +esp toolchain; deferred.
        return None;
    }
    match platform_feature(&plan.build.board, &plan.build.target) {
        Some("platform-nuttx") => Some(
...
fn generated_default_features(build: &PlanBuildOptions) -> Vec<String> {
    ...
    // Phase 126.M5.esp32 — ESP32 boards forward to `platform-bare-metal`
    // (above) but ALSO need the local `platform-esp32-qemu` feature to
    // gate the esp-hal `#[main]` entry + no_std/no_main attrs in
    // main.rs.jinja. `esp32_chip()` is the discriminator since
...
fn platform_feature(board: &str, target: &str) -> Option<&'static str> {
    match board {
        ...
        // Phase 126.M5.esp32 — ESP32 (esp-hal bare-metal) maps onto the
        // generic `platform-bare-metal` nros feature; the chip-specific
        // esp-hal deps + linker glue are driven separately via
        // `esp32_chip()`.
...
/// Phase 126.M5.esp32 — chip selection for ESP32 boards. Returns the
/// esp-hal chip feature string (`esp32c3` / `esp32s3`). `None` for
/// non-ESP32 boards. ESP32-C3 runs under QEMU's Espressif fork on
/// `riscv32imc-unknown-none-elf`; ESP32-S3 needs the `+esp` Xtensa
/// toolchain (`xtensa-esp32s3-none-elf`).
fn esp32_chip(board: &str) -> Option<&'static str> {
    match board {
        "esp32-qemu" | "esp32" | "esp32c3" | "esp32-c3" => Some("esp32c3"),
        "esp32s3" | "esp32-s3" => Some("esp32s3"),
        _ => None,
    }
}

/// Phase 126.M5.esp32 — rustc target triple for an ESP32 chip.
